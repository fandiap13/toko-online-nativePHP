...
if (isset($_POST['hapuspass'])) {
  ...
  $result = mysqli_num_rows($query);
	if ($result == 0) {
		$pesan_error = "Akun dengan E-mail <b>$email_user</b> tidak terdaftar sebagai admin pada sistem.";
  } else {
    ...
    $isi_email .= $lokasi."/admin/ubahpassword.php?email=$email_user&token=$token_ganti_pass";

    mysqli_query($conn, "UPDATE `users` SET `token_ganti_pass` = '$token_ganti_pass' WHERE email_user = '$email_user'");

    $proses_berhasil = "Link ganti password sudah dikirim ke email <b>$email_user</b>";

    $suksess = email($email_user, $judul_email, $isi_email, $proses_berhasil);
                ...
                <div class="border">
                    ...
                    <form method="POST">
                      ...
                      <p>Masukkan email akun yang ingin dihapus passwordnya.</p>
                            <input type="email" placeholder="E-mail" class="border p-3 w-100 my-2" name="email_user" value="<?= $email_user; ?>" required>
